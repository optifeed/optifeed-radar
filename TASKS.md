...
- [ ] **BUG found by the smoke test: OpenAI answers are tagged `grounded` without having searched.** On the live grounded run, **7 of 8 OpenAI answers had zero `fanoutQueries` AND zero citations** - the model chose not to search and answered from its weights - yet all 8 carry `kind: 'grounded'` and so earn M7's **1.5x grounded weight** in the composite. That premium exists to reward retrieval-backed answers, so it is unearned here (rule #6, fake precision), and the retrieval-variance marker also tells the user openai "rewrites your prompt before searching" when it did not. The 2026-07-18 `--grounded` fix addressed CAPABILITY (`supportsGrounded` - can this provider ground?) but not ACTUALITY (did this call ground?) - the same "fixing the instance is not fixing the rule" shape as the judge-selection bug. Evidence that a per-answer signal is safe: on the same run gemini showed 7/7 searched + 7/7 cited and perplexity 8/8 cited (it never exposes queries), so "no searches AND no citations" isolates exactly the 7 openai answers without touching a genuinely grounded engine. Fix = derive per-answer `kind` from evidence where the provider exposes it. NOT done here because it changes what the headline number means: it needs a `SCORING_VERSION` bump and golden updates. Impact on that run was small by luck (composite 63 -> 64, since openai's 58 sat near the composite); an engine scoring far from the mean would distort it much more.
