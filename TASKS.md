...
- [ ] **`--max-cost` costs coverage: reservations stay at the ESTIMATE even after the real cost is known (found 2026-07-23 at the smoke run, THREE independent reproductions).** `--max-cost 0.30` spent $0.048 (16% of the cap) and refused 4 of 8 prompts; `--max-cost 0.60` spent $0.33 and refused gemini 4/8; `max_cost 0.45` over MCP spent $0.24 and refused openai 4/8 and **gemini 6/8**, so that report's `gemini: 100/100` rests on 2 answers and still feeds the headline 70. Mechanism, confirmed arithmetically: `gpt-5.4` has no per-model `avgOutputTokens` so it inherits the global 2600, giving $0.0395 per call against a measured $0.0117 actual - and `runner.ts` reserves `Math.max(estimateCall(model, willSearch) * margin, observedAvg)`, so once the estimate exceeds reality the observed figure can NEVER take over, despite the comment three lines above saying "once a real cost is known the observed figure takes over, so a long run does not keep over-reserving and refusing calls it can afford". The code contradicts its own documented contract. **No hard rule is broken** - it never overspends (#5) and always reports `partialEngines` (#6) - but the cap is the product's "you control spend" premise, and today setting one at 3x the real cost still halves the sample. Options: drop the `Math.max` floor once `observedCount > 0` (what the comment already promises), or measure per-model `avgOutputTokens` for the ask path the way gemini/anthropic already have overrides. Decide before launch: a user who sets a cap generously and still gets half a run will read it as the tool being broken.
