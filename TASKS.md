...
- [ ] **Not yet live-verified with real money.** The spend figures above are exercised end to end through the CLI with mocked adapters and verified on built code, but no real-key run has printed a real bill yet. Fold into the M17 smoke test: confirm the reported total matches what the providers actually charge (Gemini grounded especially, now that per-search fees are modelled).
