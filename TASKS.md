...
- [x] **MCP ships** - stdio server over the same M10 core (M15). 4 tools at M15 (check_visibility / audit_store / generate_buyer_queries / get_snapshot_diff), FIVE since M12a added `shopping_check` on 2026-07-23; non-interactive with a default $0.50 cap; integration + schema-snapshot + failure-mode tested, and live-verified over real stdio (initialize + tools/list). **Live multi-engine `check_visibility` VERIFIED 2026-07-23** over real stdio with all four keys: score 70/100, `schema_version` 0.2, $0.2443, `serverInfo` echoing `optifeed-radar 0.1.0`, stdout protocol-clean, and honest `partialEngines` reporting on the cost cap.
...
- [x] **Competitor discovery returned the brand ITSELF under variant spellings - FIXED 2026-07-23** (found 2026-07-20 during the judge measurement). `gpt-5.4` returned "Dore Müzik" and "Do Re Müzik Market", `gemini-flash-latest` returned "Do Re Mi Müzik" - all doremusic. `buildPrompt` said "Do not include the brand itself" but passed only `brand: doremusic`; the profile's `aliases` never reached the prompt, and there was no post-filter at all (`clean()` only de-duped), so the brand landed in its own share-of-voice table as a rival, inflating a competitor that IS you (rule #6). Fixed on both sides, since neither alone is enough: the prompt now carries an `Also known as:` line built from the extracted aliases and asks the judge to exclude variant spellings AND translations, and a new pure `dropSelfReferences(names, terms)` filters the answer through the shared Unicode-aware matcher (`core/text.ts`, the same one scoring's detector uses) - applied in BOTH directions, since the returned name may wrap a brand term ("Do Re Müzik Market") or be wrapped by one, plus a squeezed comparison (`fold` + strip non-alphanumerics) that catches respacing, which is the actual shape of the live failures ("Do Re Music" -> `doremusic`). Whole terms only, so "Ace Rental" survives a brand called "Ace Hardware". Wired at the call site in the same change (`discover.ts` passes `signals.aliases`) - lesson #3, a filter nothing calls is not a fix - and pinned by a discover-level test, not only a unit test. **The self-code-review of this fix found a second defect in the first draft, fixed before commit:** filtering ran AFTER the 8-name cap, so self-references ate competitor slots - a judge opening with three spellings of the brand returned 5 rivals when 8 were sitting further down its own list (measured, not theorized). The cap is now applied to what is KEPT (`parseCompetitors(text, selfTerms)` filters before slicing), pinned by a test that was mutation-checked against the old order. **Two limits, both accepted and documented rather than papered over:** (1) replayed against the real captured strings on the BUILT code, the filter drops "Dore Müzik", "Do Re Müzik Market" and `doremusic.com`, but NOT "Do Re Mi Müzik" - a translated variant no matcher can recognize without being told, which is exactly what the prompt-side alias listing addresses and why the fix is two-sided; (2) a brand whose domain stem is one generic word (`ace.com` -> alias "ace") will over-match and drop a rival containing that word, the same ambiguity M7 flags on generic-word brands, and unlike M7 the drop here is silent. Note also that the fix only applies to FRESH discovery: a `profile.json` written before today keeps its self-references until `--refresh`. +7 tests (699 -> 706).
...
- [x] **Competitor QUALITY in non-English markets - CLOSED 2026-07-23.** Found 2026-07-17 under `gpt-4o-mini`: the Turkish list was mixed, "Muzik Aletleri" is the common noun "musical instruments" rather than a brand, and Zuhal Muzik (the obvious real rival) was absent. Re-checked live today on the same domain after the measured `JUDGE_PREFERENCE` ranking replaced the price rule: discovery returned 8 real Turkish music shops (Zuhal Muzik, MyDukkan, Cangoz Muzik, Senkop Muzik, Lale Muzik, Muziktek, Muzik Ekspres, Marmara Muzik), no common nouns, no self-references, and Zuhal Muzik ranked first. The judge-quality problem was the judge, and choosing it by measured recall fixed it.
- [x] **For RETAIL brands the share-of-voice axis was structurally mismatched - FIXED 2026-07-23 (M5a).** Found 2026-07-17, confirmed twice more: `doremusic` is a retailer, so buyer prompts like "2026 icin en iyi akustik piyanolar hangileri?" elicit MANUFACTURERS. Measured twice on 4 engines: the 2026-07-20 run (32 answers) named Yamaha 21 / Roland 16 / Casio 10 / Kawai 8 / Fender 5 / Korg 2, and the 2026-07-23 smoke run (28 answers) named Yamaha 13 / Roland 11 / Casio 8 / Kawai 8 / Fender 2 / Korg 1. **Across both runs not one rival retailer was named even once** - every shop in the share-of-voice table sat at 0 and the score was 0/100 both times. A brand in the same market and language (wefood.com.tr) did register a competitor mention, so it was the AXIS, not the locale and not the judge. **Fix (spec: docs-repo `dev-plan.md` section M5a; plan: `docs/superpowers/plans/2026-07-23-retailer-seller-axis.md`):** the one existing discovery call now also classifies the business (`businessType: retailer | maker | service`, persisted with a source so a user edit wins, every unknown falling back to the maker path that shipped before), and M5 keys the pack off it - a `where-to-buy` intent weighted 3, `best-of` DROPPED as known-zero, and per-axis guidance rewording `comparison`/`problem` as shop questions. Scoring math, `SCORING_VERSION` and `schema_version` untouched; a retailer's re-run reads as `promptSetChanged` through existing machinery. **LIVE-VERIFIED 2026-07-23 on www.do-re.com.tr** ($0.0839, openai, `--refresh --regenerate`): classified `retailer` and returned 8 rival SHOPS; the pack led with "Istanbul'da baslangic seviyesi akustik gitari hangi muzik magazasindan satin alabilirim?"; and the share-of-voice table finally names shops - **Zuhal Muzik 5 (33.3%), MyDukkan 5 (33.3%), Senkop Muzik 4 (26.7%), doremusic 1 (6.7%), score 10/100**. The 10 is a real finding where the 0 was noise: the shop is named once for every five mentions of its biggest rival. Makers still appear in passing (Yamaha 4, Roland 2 of 8 answers) but are no longer the comparison set.
...
Owner: setup · PR: #1 (merged 2026-07-23, the first module to land through a
review; earlier modules were committed straight to main)
...
- [x] **Live multi-engine + `--grounded` shopping run DONE 2026-07-23** (breville.com, 2 products, all 4 engines, 32 answers, $0.7014, clean run - no cost cap, no skipped engine). The delta table led as designed: Barista Express merchantRank 1 -> aiRank 2 (delta +1, visibility 63), Bambino Plus merchantRank 2 -> aiRank 1 (delta -1, visibility 98) - the merchant's own #1 is not what engines recommend first. Anthropic was honestly tagged `parametric` under `--grounded` (the no-mislabel guard, live), openai/gemini/perplexity `grounded`.
...
- [x] **CI matrix DONE 2026-07-22** - ubuntu/macOS/Windows x Node 20/22, `fail-fast: false`. Windows earns its slot: the CLI resolves state paths from cwd and HOME. `format:check` runs on ubuntu only (line endings differ on a Windows checkout, and a format gate is a single-platform concern). **OBSERVED GREEN 2026-07-22 on the real runners, after 4 rounds** (run 29948102376, all six legs). It paid for itself immediately: the first run failed BOTH Windows legs with 9 failures, and every cause was in test infrastructure - shipped code was never wrong on Windows. (1) Three fake filesystems matched paths with a hardcoded `/` while core joins with `node:path` (`\` on Windows), so `readdir` matched nothing and `diff`/`sources` printed an empty string. (2) The queries golden is compared byte-for-byte against generated YAML and a Windows checkout rewrites it to CRLF - a failure whose diff looked line-for-line IDENTICAL. Fixed with `.gitattributes` `eol=lf`, which covers every text fixture rather than the one that broke. **Two rounds were burned on partial fixes, which is the lesson worth keeping:** round 2 normalized the fake-fs accessors but not the SEED KEYS (built by calling the same `profilePath`/`queriesPath` helpers, so they were stored under `\`-keys while lookups normalized to `/`), and left `readFile` un-normalized; round 3 fixed one `${snapshotsDir(...)}/name` assertion and missed the identical shape three lines away in another test. A sweep for the pattern - not a fix for the instance - should have been step one, the same 'fixing the instance is not fixing the rule' shape as the judge-selection and grounded-mode bugs. An attempt to simulate Windows locally by aliasing `node:path` to its win32 build was abandoned: it also breaks genuine fs reads (`getVersion` reading package.json), and it wrote four literal `\var\folders\...` files into the repo root, caught by `git status` before they could be committed. Product code is unchanged across all four rounds; `resolveStateDir`'s POSIX joins are deliberate and valid on Windows (Node accepts mixed separators).
...
- [x] **DECIDED + MEASURED 2026-07-22: the sub-minute promise is scoped to `audit`, and `check` publishes measured ranges instead of a promise.** Reasoning: `audit` is the zero-key first command and is entirely under our control; `check`'s duration is dominated by provider latency we do not control, so any promise about it would be a claim about OpenAI's and Google's servers. A cold `npx ... audit` was measured end to end at **about 10 seconds** - 7.4s install (empty npm cache, installing from the real packed tarball) plus 0.3-1.7s for the audit itself across stripe.com / nike.com / doremusic.com / wikipedia.org. That beats the <60s gate by roughly 5x, so the gate is kept and is now backed by a number rather than a hope. The clone path was measured the same way: 7.7s cold-cache `npm install` (dev deps included) + ~2s audit, so the README's "60-second setup" heading is true with a wide margin. `check --quick` is 47-51s parametric (which would sit uncomfortably at ~55-58s cold, too close to promise) and about 97s grounded, so the README now carries a measured TIME table beside the measured COST table, plus the honest reason (provider latency) and the mitigation that already exists (`check` streams live per-phase progress to stderr, so the wait is never a blank terminal). Guarded against drift by two new copy-lint rules: "results in seconds" and "instant results" are banned across every surface, with red/green tests - the audit's speed is stated as its measured number, never as an adjective. Remaining timing work: none for the gate itself; the cold `npx` from the REGISTRY still has to be re-run once the package is published (the install measured here came from a local tarball, which skips the registry fetch of the package itself, worth well under a second at 166 kB).
- [x] **Repo created + pushed 2026-07-22: `github.com/optifeed/optifeed-radar`** (private until launch; owner's call). `main` pushed with all history; `origin` tracks it. Pre-push secret sweep: no `.env` in 111 commits (only `.env.example`), no key-shaped strings anywhere in the tree, no `.optifeed/`or `dist/` tracked, 176 files. The new slug exposed a real publish blocker: `package.json` had NO `repository` field, and npm generates the provenance attestation against it, so `npm publish --provenance` would have failed at the tag - added with `bugs`/`homepage`, pinned by tests. Cross-repo: the docs' Global decision said `optifeed/visibility`, and the SAME stale slug was in the drafted day-0 launch post (it would have shipped as a 404); both fixed (docs-repo `9a48656`).
- [x] **`--max-cost` cost coverage: reservations stayed at the ESTIMATE even after the real cost was known - FIXED 2026-07-23.** Found at the smoke run with THREE independent reproductions: `--max-cost 0.30` spent $0.048 (16% of the cap) and refused 4 of 8 prompts; `--max-cost 0.60` refused gemini 4/8; `max_cost 0.45` over MCP answered openai 4/8 and **gemini 2/8**, so that report's `gemini: 100/100` rested on two answers and still fed the headline 70. Mechanism: `projectedCost` reserved `Math.max(estimateCall(model, willSearch) * margin, observedAvg)`, so the observed figure only took over when it was HIGHER - the exact opposite of the comment above it promising that "once a real cost is known the observed figure takes over, so a long run does not keep over-reserving and refusing calls it can afford". An estimate that ran high could never be corrected by reality: `gpt-5.3-chat-latest` estimates $0.0790 a call against a measured ~$0.0117, so the cap rationed against a number **6.75x too big**. **Fix:** a blind call keeps the 1.5x margin on the estimate; once an engine has answered, reserve the DEAREST call it actually made (max not mean, so a pricier-than-average next call still has headroom). **A searching call deliberately keeps the estimate as a floor** - there the observed max is not a trustworthy ceiling, since the per-search fee dominates and its count swings 2 to 6 while OpenAI declined to search at all on 6 of 8 answers, so a no-search answer would set a low `observedMax` and under-reserve the next six-search one; under-reserving a fee-bearing call is exactly how the cap was breached 74% on 2026-07-20. **The reproduction needed real concurrency:** an instant mock never overlaps, so the first repro attempt showed 8/8 and hid the bug; holding the fake adapter's calls open reproduced the live number exactly (4 of 8). Mutation-checked by restoring the floor. **LIVE-VERIFIED 2026-07-23** on the same command that failed in the morning (`check www.do-re.com.tr --quick --engines openai --max-cost 0.30`): **8 of 8 answers, $0.0766 spent against the $0.30 cap, no `costCapped`, no `partialEngines`** - where hours earlier the identical cap returned 4 of 8. +2 tests (726 -> 728).

- [ ] **REMAINING TO PUBLISH (the only M17 work left besides the data run):** (1) add `NPM_TOKEN` to the repo secrets - the release workflow reads `secrets.NPM_TOKEN` and the publish step fails without it; (2) `git tag v0.1.0 && git push --tags` to fire the workflow; (3) cold `npx` from the REGISTRY in a clean container (the ~10s figure was measured from a local tarball, which skips the registry fetch); (4) flip the README/SKILL npm+npx copy from pending to present tense and add the launch badges (npm version/downloads, build, stars), which M16 deliberately left out while the slug and package did not exist; (5) file the directory-listing PRs (awesome-mcp-servers / ClawHub / Smithery).
...
- [x] **Measure the real grounded search-query distribution at the smoke test - DONE 2026-07-20, see the `searchesPerGroundedCall` entry above** (7 live Gemini grounded calls: 4,2,3,4,6,3,2; median 3, mean 3.43; kept at 4). This box was left unchecked after the measurement landed; the original text follows. `searchesPerGroundedCall` = 4 rested on n=1 (one captured call, 3 searches). Fanout varies with query complexity, so capture the counts across a real `--grounded` run and replace the assumption with a measured median. Note Google also grants a free grounding allowance (5,000 prompts/month at the time of writing), so a user inside it pays no fee at all - pricing it anyway is deliberate, an estimate must not under-report.
- [ ] **100-brand data-run script (budgeted) for the launch article - DEFERRED to launch week 2026-07-22 (owner's call).** Not a pre-publish gate: it produces article evidence, not shipped capability. What it is: batch `check --quick --json` over ~100 brands, aggregate the envelopes into one stats JSON. It BLOCKS the launch post, not the package - the drafted copy in `source-of-truth.html` carries `[N]`/`[X]%` placeholders under the standing rule "nothing ships with invented figures", so the post cannot go out until this has run. Costed from the measured per-run figures: ~$9 at one engine, **$41-46 all four parametric** (the likely choice), ~$109 grounded; ~85 min sequential at 47-51s per run, less with cross-brand concurrency. Three decisions to make BEFORE writing it, none of them coding decisions: (1) which 100 brands - the list IS the finding, and a biased sample makes the headline stat meaningless; (2) how partial runs (`costCapped` / `skippedEngines` / null score) enter the aggregate - excluded or reported separately, never silently averaged in (rule #6); (3) whether "invisible in at least one engine" survives contact with the data as a claim.
...
- [x] **`--max-cost` is a bounded best-effort guarantee, not an absolute one -
      DOCUMENTED 2026-07-22** in `check --help` (pinned by a test that fails on
      the phrase "hard cap") and in the README. This box was left unchecked
      after the wording landed; the original note follows. A call's cost cannot be known until it
...
- 2026-07-23: **M12a review (PR #1)** - the first module to land through a pull
  request rather than straight to main, so the review happened on the PR in two
  passes rather than as one end-of-wave workflow run. Pass 1 (`a5e1eec`, 3
  findings): a product whose category questions went UNANSWERED (cost cap,
  failed engine) was reported as though the questions were never written, in
  the headline delta table - `SkuReport` now carries `categoryPrompts` (asked)
  beside `answers` (returned) so the three states are distinguishable; run
  notes now ride the ENVELOPE, so `--json` and the MCP output carry what the
  terminal says (an agent could not otherwise learn an eleventh product was
  dropped); and two new tests assumed POSIX separators and would have failed
  the Windows leg. Pass 2 (`74bd6fa`, CodeRabbit): **CI was RED on both Windows
  legs while the branch had been reported green** - a hardcoded `/state/shopping`
  in an MCP path assertion; a money bug where `--max-cost oops` parsed to NaN
  and every guard comparison against NaN is false, so a typo'd cap ran
  UNBOUNDED while the user believed a limit was set (`shopping` and `check`
  both now refuse an unreadable, zero or negative cap before anything spends -
  hard rule #5); a shelf-parser case where a bold editorial label ("**Overall**:
  Breville Bambino Plus") was shelved as a rival product; `ambiguous` computed
  from the product name alone, so a match made through a risky single-word
  alias skipped the judge pass; a loader vouching for top-level containers only;
  and a copy-lint hole where a waitlist link could launder a present-tense
  roadmap claim. Docs pass (`c0effd4`, `3eb9519`): METHODOLOGY.md documented the
  product-level scoring and joined the copy lint (it still said "Optifeed
  Visibility", a release after the rename). **Open:** whether M12a still owes a
  workflow-backed high-effort review of the kind every earlier wave got - the
  PR passes above were narrower in scope than a 30-agent finder sweep.

- 2026-07-23: **M17 smoke run, all four engines, $2.85 total.** Seven runs: notion.so 47/100 ($0.3302), www.do-re.com.tr 0/100 ($0.3191), optifeed.com 0/100 ($0.3539), notion.so `--grounded` 58/100 ($0.8527), a `--refresh` do-re run ($0.0485), a 4-engine `--grounded` shopping run on breville.com ($0.7014), and a 4-engine `check_visibility` over real MCP stdio ($0.2443). **Passed:** engine-phase spend equalled the sum of its answers' `costUsd` on every single run (no under-reporting); anthropic honestly tagged `parametric` under `--grounded`; `serverInfo` echoed `optifeed-radar 0.1.0` with clean stdout; the shopping delta table worked across all four engines. **Measured:** grounded searches per call - openai 4,2 (n=2) and gemini 2,2,2,2,2 (n=5), against the `searchesPerGroundedCall` assumption of 4, which stays conservative in the safe direction. The grounded run also re-confirmed the SCORING_VERSION 3 fix earning its keep: 6 of 8 openai and 3 of 8 gemini answers were tagged grounded having run no search and returned no citation, so under v2 they would each have drawn an unearned 1.5x weight. **Found:** the `--max-cost` over-reservation item above, reproduced three times. **Also cleared:** M12a's last box and the MCP live-verification box.

- 2026-07-23: **self-review of the M5a commits, 2 findings, both fixed test-first** (+2 tests, 724 -> 726). (1) **A parser regression I had just introduced.** The new object branch of the discovery parse matched its keys case-sensitively and returned unconditionally, so `{"Competitors": [...]}` or `{"rivals": [...]}` came back with ZERO competitors where the array parser it replaced read them fine - verified against the built code, not just reasoned about. This is the same class the M14 review already fixed once (`{"Products":[...]}` read as 0 products), which is the lesson worth keeping: an object branch added in front of a working parser must never be a WORSE parser than the one it shadows. Fixed by matching keys case-insensitively, accepting snake_case `business_type` (a silently dropped classification means the seller axis never engages and a shop is scored on maker questions again - the exact bug M5a exists to fix), and letting the object branch win ONLY when it found something, so anything unanticipated falls through to the array parser. (2) **Fake precision in published copy.** The counts Yamaha 21 / Roland 16 / Casio 10 / Kawai 8 are from the 2026-07-20 run (32 answers); I had attributed them to the 2026-07-23 smoke run, which measured 28 answers and counted Yamaha 13 / Roland 11 / Casio 8 / Kawai 8. Both runs support the finding - not one rival shop was named in either, verified independently - but quoting one run's numbers under another run's label is the invented precision the copy rules forbid, and it had reached METHODOLOGY.md, which ships. Corrected in METHODOLOGY.md, TASKS.md and the docs-repo spec. Noted and NOT fixed: `intentQuotas` returns NaN for an all-zero-weight intent set (unreachable through `activeIntents`, not in the public barrel, so a guard would be speculative code), and a judge returning literally `{}` yields a competitor named "{}" (pre-existing, the old parser did the same). **Caveat recorded honestly: the parser fix landed AFTER the live do-re verification, so that run used the pre-fix parser. The fix only widens what parses and the canonical shape is unchanged and still covered, so no re-run was done - a judgment call, not a proof.**
